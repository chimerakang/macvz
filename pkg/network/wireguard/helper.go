...
// (cmd/macvz-netd) over its unix socket, so the user-run kubelet can manage the
// WireGuard interface and routes without itself being root (#38).
...
// WithHelperSocket routes the Mesh's privileged commands through the root helper
// daemon at socketPath instead of executing them directly. Use this when the
// kubelet runs as a non-root user (#38).
func WithHelperSocket(socketPath string) Option {
	return func(m *Mesh) {
		m.run = helperRunner{client: privhelper.NewClient(socketPath)}
	}
}
