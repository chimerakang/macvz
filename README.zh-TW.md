...
- **簽章與 notarization。** MacVz 發佈用 binary 必須正確簽章（對外分發還需 notarization）。kubelet 透過 CLI 驅動 `apple/container`，沒有直接 link Virtualization.framework，因此 MacVz 自己的 entitlement 檔目前刻意保持空白；除非架構改成直接使用 VZ，才需要新增虛擬化 entitlement。
